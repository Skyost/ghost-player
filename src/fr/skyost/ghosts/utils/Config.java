...
public abstract class Config extends ConfigObject {
	...
	public Config load(final File file) throws InvalidConfigurationException {
		if(file==null) {
	...
	public Config reload() throws InvalidConfigurationException {
		...
		final YamlConfiguration yamlConfig = YamlConfiguration.loadConfiguration(CONFIG_FILE);
		try {
			onLoad(yamlConfig);
	...
	public Config save(final File file) throws InvalidConfigurationException {
		if(file==null) {
			throw new InvalidConfigurationException(new NullPointerException());
		}
		CONFIG_FILE = file;
		...
		if(!CONFIG_FILE.exists()) {
			try {
				...
				if(CONFIG_HEADER!=null) {
					final Writer newConfig = new BufferedWriter(new FileWriter(CONFIG_FILE));
					for(final String line : CONFIG_HEADER.split("\n")) {
	...
	public Config init(final File file) throws InvalidConfigurationException {
		if(file==null) {
			throw new InvalidConfigurationException(new NullPointerException());
		}
		CONFIG_FILE = file;
